...
describe 'compiled component acm' do
  ...
  context 'cftest' do
    it 'compiles test' do
      expect(system("cfhighlander cftest #{@validate} --tests tests/alternative_names.test.yaml")).to be_truthy
    end      
  end
  
  let(:template) { YAML.load_file("#{File.dirname(__FILE__)}/../out/tests/alternative_names/acm.compiled.yaml") }
  ...
  context "Resource" do
    ...
    context "ACMCertificate" do
      ...
      it "to have property AlternativeNames" do
          expect(resource["Properties"]["AlternativeNames"]).to eq(["test.com", "api.test.com"])
      end
